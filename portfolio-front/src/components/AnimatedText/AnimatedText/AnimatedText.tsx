...
const AnimatedText: FC<IAnimatedText> = ({
    ...
    const { getDataSequence } = useParsedNodes();
    const textSequence = getDataSequence(innerHtml);
    ...
    return (
        ...
        >
            {textSequence.map((item, seqI) => {
                        ...
                        <Words key={wi}>
                            <Word>
                                {word.split("").map((letter, li) => {
                                    return letter === " " ? (
                                        <Space {...animations.letter} key={li}>
                                            &nbsp;
                                        </Space>
                                    ) : (
                                        <Letter key={li} {...animations.letter}>
                                            {letter}
                                        </Letter>
                                    );
                                })}
                ...
                if (item.type === EMarkup.Em)
                    return <em key={seqI}>{result}</em>;
